...
import type { LessonPlan, Day } from '../_types'
...
const EMPTY_FORM = {
  title: '', subject: '', group: '',
  week: '', day: 'Monday' as Day, period: 1,
...
export default function LessonPlanSection() {
  ...
  // Set default group/subject in form once groups load
  useEffect(() => {
    if (groups.length > 0 && !form.group) {
      setForm(f => ({ ...f, group: groups[0].name, subject: groups[0].subject }))
    }
  }, [groups])
  ...
  const weeklyPlans: Record<Day, LessonPlan[]> = {
    Monday: [], Tuesday: [], Wednesday: [], Thursday: [], Friday: [],
  }
  filtered.forEach((p: LessonPlan) => { if (weeklyPlans[p.day]) weeklyPlans[p.day].push(p) })

  return (
    <div>
      <PageHeader title="Lesson Plans"
        subtitle="Create weekly and unit lesson plans aligned to the curriculum."
      ...
      <div className="px-6 pb-8 space-y-4">
        ...
        {selectedPlan ? (
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-medium px-2 py-0.5 rounded-full"
                    style={{ background: statusStyle[selectedPlan.status]?.bg, color: statusStyle[selectedPlan.status]?.text }}>
                    {statusStyle[selectedPlan.status]?.label}
                  </span>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 animate-in stagger-2">
            {isLoading && <p className="col-span-full text-center py-8 text-sm" style={{ color: '#6B6660' }}>Loading lesson plans...</p>}
            {!isLoading && filtered.map((plan: LessonPlan) => {
              const sStyle = statusStyle[plan.status] ?? statusStyle.draft
              return (
            ...
            {DAYS_LIST.map(day => (
              <div key={day} className="card" style={{ minHeight: 200 }}>
                <h3 className="text-xs font-bold uppercase tracking-widest mb-3 pb-2"
                  style={{ color: '#6B6660', borderBottom: '1px solid #E4E1D8' }}>{day}</h3>
          ...
          <div className="card w-full max-w-2xl mx-4 animate-in">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { setShowCreateModal(false); resetForm() }}
                className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-gray-100"><X size={18} /></button>
            </div>
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.group}
                    onChange={e => {
                      const g = groups.find((x: any) => x.name === e.target.value)
                      setForm(f => ({ ...f, group: e.target.value, subject: g?.subject ?? f.subject }))
                    }}
                    className="select">
                    {groups.map((g: any) => <option key={g.id} value={g.name}>{g.name}</option>)}
                  </select>
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <input value={form.week} onChange={e => setForm(f => ({ ...f, week: e.target.value }))} className="input" placeholder="e.g. Week 21" />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea value={form.homework} onChange={e => setForm(f => ({ ...f, homework: e.target.value }))} className="input" rows={3} placeholder="Describe the homework..." />
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => { setShowCreateModal(false); resetForm() }} className="btn-outline">Cancel</button>
              <button onClick={() => { setShowCreateModal(false); resetForm() }} className="btn-gold" disabled={!form.title}>
