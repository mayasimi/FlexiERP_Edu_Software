...
export default function PerformanceSection() {
  ...
  const { data: performance = {
    stats:             { avg_score: 0, attendance_rate: '0%', top_performers: 0, at_risk: 0 },
    top_performers:    [],
    at_risk:           [],
    weekly_attendance: [],
    score_trends:      [],
    ...
    queryKey: ['teacher-performance', selectedGroup],
    queryFn:  () => teacherApi.getPerformance(selectedGroup !== 'all' ? selectedGroup : undefined).then(r => r.data),
  })
    ...
    <div>
      ...
      <div className="px-6 pb-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-in stagger-1">
          <StatCard icon={<BarChart3 size={18} style={{ color: '#C9A020' }} />} iconBg="rgba(201,160,32,0.12)" value={performance.stats.avg_score}      label="Avg Score" />
          <StatCard icon={<TrendingUp size={18} style={{ color: '#10B981' }} />} iconBg="rgba(16,185,129,0.12)"  value={performance.stats.attendance_rate} label="Attendance Rate" />
          <StatCard icon={<Award size={18} style={{ color: '#3B82F6' }} />}      iconBg="rgba(59,130,246,0.12)"  value={performance.stats.top_performers}  label="Top Performers" />
          <StatCard icon={<TrendingDown size={18} style={{ color: '#EF4444' }} />} iconBg="rgba(239,68,68,0.12)" value={performance.stats.at_risk}          label="At Risk" />
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div className="card animate-in stagger-2">
            ...
            {performance.score_trends.length === 0 ? (
              <p className="text-center py-8 text-sm" style={{ color: '#6B6660' }}>No trend data available yet.</p>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={performance.score_trends}>
          ...
          <div className="card animate-in stagger-3">
            ...
            {performance.weekly_attendance.length === 0 ? (
              <p className="text-center py-8 text-sm" style={{ color: '#6B6660' }}>No attendance data available yet.</p>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={performance.weekly_attendance}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E4E1D8" />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          {/* Subject Performance — from API or empty state */}
          <div className="card animate-in stagger-3">
            <h3 className="font-bold text-base mb-4">Subject-wise Performance</h3>
            {(performance.subject_performance ?? []).length === 0 ? (
              <p className="text-sm" style={{ color: '#6B6660' }}>No subject data yet.</p>
            ) : (
              <div className="space-y-3">
                {(performance.subject_performance ?? []).map((sub: any, idx: number) => (
                  <div key={idx}>
                    ...
                    <div className="w-full h-2 rounded-full" style={{ background: '#F3F4F6' }}>
                      <div className="h-2 rounded-full transition-all"
                        style={{ width: `${sub.avgScore}%`, background: sub.avgScore >= 75 ? '#10B981' : sub.avgScore >= 60 ? '#C9A020' : '#EF4444' }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="card animate-in stagger-4">
            ...
            {performance.top_performers.length === 0 ? (
              <p className="text-sm" style={{ color: '#6B6660' }}>No data yet.</p>
            ) : (
              <div className="space-y-3">
                {performance.top_performers.map((student: any, idx: number) => (
                  <div key={student.id} className="flex items-center gap-3 p-2 rounded-lg"
                    style={{ background: idx === 0 ? 'rgba(201,160,32,0.06)' : 'transparent' }}>
            ...
            <h3 className="font-bold text-base mb-4">
              <span className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ background: '#FEF2F2', color: '#991B1B' }}>
                  {performance.at_risk.length}
                </span>
              </span>
            </h3>
            {performance.at_risk.length === 0 ? (
              <p className="text-sm" style={{ color: '#10B981' }}>No at-risk students. 🎉</p>
            ) : (
              <div className="space-y-3">
                {performance.at_risk.map((student: any) => (
